...
window.addEntry = function addEntry() {
  ...
  if (key && value) {
    set(ref(database, "template/" + key), value).then(() => {
...
window.deleteEntry = function deleteEntry(key) {
  if (confirm(`Are you sure you want to delete "${key}"?`)) {
    remove(ref(database, "template/" + key)).then(() => {
...
window.editEntry = function editEntry(key) {
  ...
  get(ref(database, "template/" + key)).then(snapshot => {
...
window.saveEdit = function saveEdit() {
  ...
  if (currentEditKey && newKey && newValue) {
    ...
    if (currentEditKey !== newKey) {
      remove(ref(database, "template/" + currentEditKey)).then(() => {
        set(ref(database, "template/" + newKey), newValue).then(() => {
      ...
      set(ref(database, "template/" + currentEditKey), newValue).then(() => {
        alert("Posten uppdaterad!");
        closeModal();
        loadEntries();
      }).catch(error => {
