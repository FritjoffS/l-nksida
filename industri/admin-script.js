    ...
    function loadEntries() {
      ...
      database.ref("industri").once("value").then(snapshot => {
    ...
    function addEntry() {
      ...
      if (key && value) {
        const database = firebase.database();
        database.ref("industri/" + key).set(value).then(() => {
    ...
    function deleteEntry(key) {
      if (confirm(`Are you sure you want to delete "${key}"?`)) {
        const database = firebase.database();
        database.ref("industri/" + key).remove().then(() => {
    ...
    function editEntry(key) {
      ...
      database.ref("industri/" + key).once("value").then(snapshot => {
    ...
    function saveEdit() {
      ...
      if (currentEditKey && newKey && newValue) {
        ...
        if (currentEditKey !== newKey) {
          database.ref("industri/" + currentEditKey).remove().then(() => {
            database.ref("industri/" + newKey).set(newValue).then(() => {
          ...
          database.ref("industri/" + currentEditKey).set(newValue).then(() => {
            alert("Posten uppdaterad!");
            closeModal();
            loadEntries();
          }).catch(error => {
