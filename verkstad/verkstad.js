...
async function saveLink(event) {
  ...
  try {
    ...
    if (currentEditKey && currentEditKey !== name) {
      const oldRef = ref(db, "verkstad/" + currentEditKey);
      await remove(oldRef);
    }

    // Save the new/updated entry as an object
    const linkRef = ref(db, "verkstad/" + name);
    const linkData = {
      url: url,
...
async function deleteLink() {
  ...
  try {
    const linkRef = ref(db, "verkstad/" + currentEditKey);
...
// Show empty state
function showEmptyState(message = 'Inga verkstadsslänkar tillgängliga') {
  if (buttonContainer) {
    buttonContainer.innerHTML = `
